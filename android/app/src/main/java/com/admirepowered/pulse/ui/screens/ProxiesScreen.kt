        ...
        items(groups, key = { it.name }) { group ->
            Column(
                ...
                Row(
                    ...
                    AssistChip(
                        onClick = { },
                        label = { Text(group.type.ifBlank { "Selector" }) },
                    )
                }
                group.proxies.forEach { proxy ->
                    Surface(
                        ...
                        modifier = Modifier.clickable { onProxySelect(proxy.id) },
                    ) {
                        PulseRow(
                            title = proxy.name,
                            subtitle = if (proxy.selected) "当前选择" else group.selectedName.ifBlank { proxy.group },
                            trailing = {
