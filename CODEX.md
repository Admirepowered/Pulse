...
- Service-launched Pulse must remain hidden through startup and DOM ready. `PulseStartupService` should wait briefly for the Windows shell tray before launching the user-session UI, otherwise the tray icon can register as an empty/white square during early login.
...
- If `AutoStartService` is enabled, the installed helper service number is older, and Pulse is already running with administrator rights, startup should automatically sync/re-register the service helper instead of only showing a notice.
- Pulse truncates app/core/service log files on process startup; do not keep daily marker-based cleanup unless the user asks for historical logs.
